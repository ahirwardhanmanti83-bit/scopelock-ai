...
export const ChangeOrderPreview: React.FC<ChangeOrderPreviewProps> = ({ report, onSendEmail }) => {
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(changeOrderText);
    setCopied(true);
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!isUnlocked ? (
            <button
              ...
              className="px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-emerald-600 via-indigo-600 to-purple-600 hover:opacity-95 rounded-xl flex items-center gap-2 shadow-lg shadow-indigo-600/30 transition-all cursor-pointer animate-pulse"
            >
              <Sparkles className="w-4 h-4 text-amber-300" />
              <span>Unlock Full Change Order ($2 USD)</span>
            </button>
          ) : (
            <>
              <button
      ...
      <div className="mt-4 relative">
        ...
        {!isUnlocked && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/70 backdrop-blur-[2px] rounded-xl p-6 text-center">
            ...
            >
              <Sparkles className="w-4 h-4 text-amber-300" />
              <span>Unlock Instantly ($2 USD)</span>
            </button>
          </div>
        )}
      </div>
