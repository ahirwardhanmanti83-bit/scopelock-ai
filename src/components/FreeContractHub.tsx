...
export const FreeContractHub: React.FC<FreeContractHubProps> = ({ onUnlockTool, onOpenLicenseModal }) => {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(selectedTemplate.fullMarkdown);
    setCopied(true);
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 sm:p-5 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-900 pb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 transition-colors cursor-pointer"
            >
