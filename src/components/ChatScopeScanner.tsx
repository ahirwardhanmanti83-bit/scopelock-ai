    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3 flex flex-col justify-between">
          ...
          <button
            type="button"
            onClick={() => handleCopy(detectedCreep.diplomaticReply, 'diplomatic')}
            className="w-full py-2 px-3 rounded-lg bg-slate-850 hover:bg-slate-800 text-slate-200 font-semibold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
          >
            {copiedReply === 'diplomatic' ? (
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            ) : (
              <Copy className="w-3.5 h-3.5" />
            )}
            <span>{copiedReply === 'diplomatic' ? 'Copied to Clipboard!' : 'Copy Diplomatic Reply'}</span>
          </button>
        </div>
        ...
        <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3 flex flex-col justify-between">
          ...
          <button
            type="button"
            onClick={() => handleCopy(detectedCreep.firmReply, 'firm')}
            className="w-full py-2 px-3 rounded-lg bg-slate-850 hover:bg-slate-800 text-slate-200 font-semibold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
          >
            {copiedReply === 'firm' ? (
            ...
            <span>{copiedReply === 'firm' ? 'Copied to Clipboard!' : 'Copy Legal Notice Reply'}</span>
          </button>
        </div>
      </div>
    </div>
